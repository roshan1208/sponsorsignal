...
const AMBIGUOUS_JOB_TITLES = new Set([
  'it','hr','qa','tech','technology','manager','assistant','officer','worker','staff','consultant','analyst','engineer'
]);
...
function jobTitleIssue(value){
  ...
  if((letters < MIN_JOB_TITLE_LETTERS && !recognisedShortTitle) ||
    (words.length === 1 && AMBIGUOUS_JOB_TITLES.has(words[0]))){
    return 'This title is too broad to identify an occupation. Enter a fuller vacancy title, such as IT service manager.';
  }
  return '';
}
...
function jobBrowserBookmarklet(){
  function run(){
    ...
    window.open('https://knowyoursponsor.co.uk/?tool=job#job=' + encoded,'_blank','noopener');
  }
...
function resolveBrowserJobEmployer(){
  const api = window.jh;
  const employers = api ? api[0]() : [];
  if(!pendingBrowserJob?.employer || !employers.length) return false;
  const row = api[1](pendingBrowserJob.employer);
  if(row){
    api[2](row,true);
...
function renderBrowserJobStatus(exact=false){
  ...
  const found = [payload.employer && 'employer',payload.role && 'job title',payload.location && 'location',payload.salary && 'salary'].filter(Boolean);
...
function applyBrowserJobPayload(payload){
  ...
  const exact = resolveBrowserJobEmployer();
...
function startBrowserJob(){
  ...
  if(browserJob || requestedTool === 'job' || requestedTool === 'applications'){
    window.jh[6](null,null,browserJob ? 'check' : (requestedTool === 'applications' ? 'applications' : 'check'));
    if(browserJob) applyBrowserJobPayload(browserJob);
