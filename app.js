(function(){
  ...
  let checkedJob = null, jobFeedbackRecorded = false;
  let jobSuggestions = [];
  ...
  function render(reset){
    ...
    const html = slice.map(s=>{
      ...
      const isNew = view === '' && newNames.has(name.toLowerCase());
      ...
      const identityTag = focusedSingle && evidenceLoaded
        ? (companyRecord.number
          ? ' <span class="tag listed">Company record linked</span>'
          : ' <span class="tag limitation">Company identity not safely linked</span>')
        : '';
      const singleHelp = focusedSingle
        ...
        : '';
      const statusTags = listedTag + identityTag + (isNew?' <span class="tag new">New</span>':'') + warnTag;
      return `<tr data-name="${attr(name)}">
        <td class="name"><button class="star${saved.has(savedKey(name, town)) ? ' on' : ''}" type="button" data-save="${attr(savedKey(name, town))}"`+
          ` aria-pressed="${saved.has(savedKey(name, town))}"`+
          ` title="Save to your shortlist" aria-label="Save ${attr(cleanDisplayText(name))} to your shortlist">${saved.has(savedKey(name, town)) ? '&#9733;' : '&#9734;'}</button><button class="rowbtn" type="button" data-name="${attr(name)}">${esc(cleanDisplayText(name))}</button><span class="single-statuses">${statusTags}</span>${singleHelp}`+
          identityAliasHTML(s) +
          `<button class="compare-row${comparing.has(name) ? ' on' : ''}" type="button" data-compare="${attr(name)}" aria-pressed="${comparing.has(name)}">${comparing.has(name) ? 'Selected' : '+ Compare'}</button></td>
        <td data-label="Location">${esc(loc)}</td>
        <td data-label="Industry">${esc(industry)}</td>
  ...
  function emptyMessage(){
    if(warnOnly === 'serious'){
      return '<strong>No safely linked company record has a serious non-active status here.</strong>' +
    ...
    if(!sourceRows().length){
      if(view === 'added'){
        return `<strong>No employers were added in the last ${windowDays} days.</strong>` +
               `<br>The register does not change every day. Check back soon.`;
      }
      if(view === 'removed'){
        return `<strong>No employer lost their licence in the last ${windowDays} days.</strong>` +
               `<br>That is good news for anyone job hunting right now.`;
      }
      if(view === 'saved'){
  ...
  function cleanPlace(value){
    ...
    return /^(?:-+\s*)?select one(?:\s*-+)?$/i.test(text) ? '' : text;
  }
  ...
  function apply(){
    ...
    filtered = sourceRows().filter(row=>matchesFilters(row, q, ''));
    updateFacets(q);
    filtered = sortRows(filtered, $('q').value);
    updateSearchPresentation(filtered.length);
    render(true);
  ...
  // ----------------------------------------------------- detail sheet --
  const RATING_NOTE = {
    A: 'In good standing with the Home Office.',
  ...
  const EVIDENCE_KIND_LABELS = {
    official:'Official', derived:'Derived', unknown:'Unknown', user:'Your input'
  };
  ...
  function evidenceClaim(kind,label,value,meta){
    return '<div class="evidence-claim">' + evidenceKind(kind) +
      '<div class="evidence-claim-copy"><strong>' + esc(label) + ': ' + value +
      '</strong>' + (meta ? '<span>' + meta + '</span>' : '') + '</div></div>';
  }
  function companyCorrectionHref(name,identity){
    const number = identity && identity.number || 'Not safely linked';
  ...
  function employerEvidenceReceipt(row,identity,record,cqc,cosHistory,cosUsage){
    ...
    return '<section class="evidence-receipt" aria-label="Evidence receipt">' +
      ...
      companyIdentityPassport(name,identity) +
  ...
  function sheetHTML(row){
    ...
    const locationLabel = [cleanPlace(town), cleanPlace(county)].filter(Boolean)
      ...
      .join(', ');
    const record = companyFlags[name];
    const identity = companyIdentities.get(name);
    ...
    const parts = ['<p class="check-kicker">KnowYourSponsor Check</p>',
      '<div class="check-summary"><strong>' +
      (listingPresent ? 'Present in the latest sponsor register' :
        'This listing is not present in the latest sponsor register') +
      '</strong><span>' + (checked ? 'Checked ' + esc(checked) + '. ' : '') +
      'Always confirm against the official source before acting.</span></div>',
      '<dl>'];
    if(locationLabel) parts.push(`<dt>Location</dt><dd>${esc(locationLabel)}</dd>`);
    parts.push(`<dt>Industry</dt><dd>${esc(industry)}</dd>`);

    parts.push('</dl>');
    const cosHistory = cosHistories.get(name);
    const cosUsage = cosUsageRecords.get(name);
    parts.push(employerEvidenceReceipt(
      row,identity,record,cqcProviders.get(name),cosHistory,cosUsage));

    // Nobody else can answer this. GOV.UK publishes who holds a licence
    // today and never when it started, so it only exists because the
    // pipeline has kept a dated log.
    const started = licensedSince[(name + '|' + town).toLowerCase()];

    if(routes && routes.length){
      parts.push('<div class="block"><h3>Visa routes it can sponsor</h3>' +
    ...
    const cqc = cqcProviders.get(name);
    if(cqc && cqc.state === 'identified'){
    ...
    const checks = [];
    checks.push(['Home Office rating', rating === 'A'
      ? 'A, which is good standing'
      : (rating === 'B'
          ? 'B, so it usually cannot issue new certificates right now'
          : 'Not published')]);
    checks.push(['Licence held since', started
  ...
  function occupationSearchText(record){
    return [record.code,record.jobType,...(record.titles || [])].join(' ').toLowerCase();
  }

  function occupationRank(record, query){
    const job = record.jobType.toLowerCase();
    const titles = (record.titles || []).map(title=>title.toLowerCase());
    if(record.code === query) return 0;
    if(job === query || titles.includes(query)) return 1;
    const starts = [job,...titles].filter(title=>title.startsWith(query));
    if(starts.length) return 2 + Math.min(...starts.map(title=>title.length)) / 1000;
    const words = occupationWords(query).filter(word=>word.length > 1);
    const phrases = [record.jobType,...(record.titles || [])].map(occupationWords);
    if(words.length && phrases.some(phrase=>words.every(word=>phrase.includes(word)))) return 4;
    if(occupationSearchText(record).includes(query)) return 5;
    return 99;
  }
  ...
  function occupationMatchReason(record, query){
    const rank = occupationRank(record,String(query || '').trim().toLowerCase());
    if(rank <= 1) return 'Exact title or code match';
    if(rank < 3) return 'Official title starts with your wording';
    if(rank === 4) return 'All important title words appear in this occupation';
    return query ? 'Related official title wording' : 'Official occupation option';
  }
  ...
  function setOccupationSuggestionsOpen(open){
    ...
    $('browseOccupations').textContent = open ? 'Close matches' : 'Find official occupation';
  }

  function chooseOccupation(record, keepRole=true, confirmed=false){
    selectedOccupation = record;
    $('jobSoc').value = record.code;
    const enteredTitle = $('jobRole').value.trim();
    const resolvedTitle = resolvedOccupationTitle(enteredTitle,record.jobType);
  ...
  function showOccupationSuggestions(value, browseAll=false){
    ...
    loadOccupationData().then(document=>{
      const ranked = document.occupations
        .map(record=>({record,rank:occupationRank(record,query)}))
        .filter(item=>item.rank < 99)
        .sort((a,b)=>a.rank-b.rank || a.record.jobType.localeCompare(b.record.jobType));
      occupationSuggestions = ranked.slice(0,7).map(item=>item.record);
      if(!occupationSuggestions.length){
        $('jobOccupationSuggestions').innerHTML = '<span class="application-empty">No close official title found. Try the main duties or enter a confirmed four-digit code.</span>';
        return;
      }
      const guidance = mode === 'guided'
        ? '<span class="application-empty"><strong>Complete the title or choose a likely occupation.</strong> These matches come from your partial wording. Selecting one replaces the incomplete title; confirm it against the vacancy duties.</span>'
        : '<span class="application-empty"><strong>Possible official occupations</strong> Compare these with the vacancy duties. These are not jobs published by this employer.</span>';
      $('jobOccupationSuggestions').innerHTML = guidance + occupationSuggestions.map((record,index)=>
        '<button type="button" role="option" data-occupation-suggestion="' + index + '"><strong>' +
        esc(record.code + ' · ' + record.jobType) + '</strong><span>' +
        esc((record.titles || []).slice(0,3).join(' · ') || 'No example titles listed') +
        '</span><span><strong>Possible match ' + (index + 1) + ':</strong> ' + esc(occupationMatchReason(record,query)) +
        '</span><span>' + esc(occupationEligibility(record)) + '</span></button>'
      ).join('');
    }).catch(()=>{
  ...
  function jobDecisionBrief(application,row,occupation,salaryGap,warning,identity,cosHistory,cosUsage){
    ...
    return '<section class="job-decision-brief ' + esc(brief.state) +
      '" aria-labelledby="jobDecisionTitle"><span class="eyebrow">What the evidence says</span>' +
  ...
  function renderJobResult(application, row){
    ...
    const salaryGap = goingRate && application.occupationConfirmed && occupationEvidenceFresh()
    ...
    const identityCard = '<section class="job-report-identity" aria-label="Vacancy checked"><span class="eyebrow">Vacancy checked</span>' +
      ...
      '<dt>Selected occupation</dt><dd>' + (occupation ? esc(occupation.code + ' · ' + occupation.jobType) : 'Not confirmed') + '</dd></dl></section>';
  ...
  function recordJobFeedback(value){
    ...
    else if(value === 'employer') trackEvent('job_feedback_employer_match','Job report employer match needs work');
    ...
    if(communityPilot && value !== 'useful') trackEvent('community_pilot_feedback_needs_work','Community pilot report needs work');
    ...
    status.textContent = value === 'useful'
      ? 'Thank you. This helps us protect what works.'
      : 'Thank you. We will use this to improve the job check.';
  }
  ...
  function employerHistory(record, row, started){
    const dates = (record && record.dates) || {};
    ...
    if(started){
      ...
      events.push({date:licensedBaseline,
        ...
        note:'The licence started before this date; the exact date is not published.'});
    }
    ...
    return '<div class="block evidence-timeline"><h3>Employer history</h3><ul>' +
      items.join('') + '</ul></div>';
  }
  ...
  function employerComparison(name){
    ...
    return {name:actualName, current:current.length>0, locations, industries, routes,
      ...
      incorporated:flag&&flag.incorporated, number:flag&&flag.number};
  }
  ...
  function renderComparison(){
    ...
    $('compareBody').innerHTML = '<table class="compare-table"><thead><tr><th>Evidence</th>' +
      ...
      '<tr><td>Industry</td>' + values(item=>esc(item.industries.join('; ')||'Other')) + '</tr>' +
      ...
      '</tbody></table><p class="caveat">A sponsor licence means an employer can sponsor on the routes shown. It does not mean the employer is hiring, that a role qualifies, or that an application will succeed. Companies House matches are pointers, not verdicts.</p>';
  }
  ...
  function setView(next){
    ...
    if(view === 'removed'){
      ...
      el.innerHTML = '<strong>These employers can no longer sponsor a visa.</strong> ' +
        `They were on the official register within the last ${windowDays} days ` +
        'but have since been removed. Applying to them for sponsored work is ' +
      ...
      if(flagged.length){
        extra = ` <strong>${flagged.length} of them already have a ` +
          ...
          '. New licence, existing question: worth checking before you apply.';
      }
      el.className = 'notice';
      el.innerHTML = '<strong>Newly licensed employers.</strong> ' +
        `Added to the register in the last ${windowDays} days. Fewer applicants ` +
        'know these exist yet, so there is often less competition.' + extra;
      el.hidden = false;
    }else if(view === 'saved'){
      el.className = 'notice';
  ...
  function restoreInputs(){
    ...
    comparing.clear();
    p.getAll('compare').slice(0,3).filter(Boolean).forEach(name=>comparing.add(name));
    syncCompareUI();
  ...
  $('sheetSave').addEventListener('click',()=>{
    if(sheetRow) toggleSaved(savedKey(sheetRow[0], sheetRow[1]));
  });
  ...
  $('jobAdvertStatus').addEventListener('click',event=>{
  ...
  $('jobOccupationSuggestions').addEventListener('click',event=>{
    ...
    if(!button) return;
    const occupation = occupationSuggestions[Number(button.dataset.occupationSuggestion)];
    if(occupation){
      chooseOccupation(occupation,true);
      $('jobSalary').focus();
    }
  });
  ...
  $('jobForm').addEventListener('submit',async event=>{
    ...
    const matches = all.filter(row=>comparableEmployerName(cleanDisplayText(row[0])) === comparableEmployerName(employer));
    ...
    }catch(error){ selectedOccupation = null; }
  ...
  function loadCompleteRegister(){
    ...
    addedRows = recent.new||[];
    removedRows = gone.removed||[];
